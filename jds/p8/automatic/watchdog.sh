...
#
# Example Usage:
# Starts the game server(s) corresponding to the given IDs.
# ./watchdog.sh 1/2/3/4/5...
#
...
[[ "$os_name" != "debian" && "$os_name" != "ubuntu" && "$os_name" != "centos" && "$os_name" != "rhel" && "$os_name" != "rocky" && "$os_name" != "almalinux" ]] && exit 1
...
send_message() {
    ...
    curl -s -X POST "https://api.honeok.com/api/log" \
        ...
        -d "{\"action\":\"$event\",\"timestamp\":\"$china_time\",\"country\":\"$country\",\"os_name\":\"$os_name\",\"cpu_arch\":\"$cpu_arch\"}" >/dev/null 2>&1 & 
}
...
if ! command -v sshpass >/dev/null 2>&1 && type -P sshpass >/dev/null 2>&1; then
    if command -v dnf >/dev/null 2>&1 || command -v yum >/dev/null 2>&1; then
        ...
        dnf install -y sshpass || yum install -y sshpass
