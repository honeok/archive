...
to_arr_fun() {
    ...
    IFS=', ' read -r -a arr <<< "${BASH_REMATCH[1]}"
    if [[ ${#arr[@]} -eq 0 ]]; then
...
limit_fun() {
    ...
    local svc_num=$2
    local host_name
    host_name=$(get_host_name "$line_num") || exit 1
    ...
    IFS=$'\n' read -r -a arr <<< "$(to_arr_fun "$line_num")" || exit 1
...
main() {
    ...
    # 处理limit
    IFS=$'\n' read -r -a arr <<< "$(to_arr_fun "$row")" || exit 1
    if [[ ${arr[0]} -eq $server_num ]]; then
        if [[ $row -gt 1 ]]; then
            local prev_row=$((row-1))
	    echo "prev_row=$prev_row"
            IFS=$'\n' read -r -a prev_arr <<< "$(to_arr_fun "$prev_row")" || exit 1
