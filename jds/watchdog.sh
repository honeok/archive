...
for timeapi in "$taobao_timeapi" "$suning_timeapi"; do
    open_server_time=$timeapi  # 将当前API返回的时间赋值给open_server_time
...
send_message() {
    local action="$1"
    local country=$(curl -s https://ipinfo.io/country || echo "unknown")
